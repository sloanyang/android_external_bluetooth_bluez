						...
						uint32_t passkey, void *cb)
{
    ...
    char adapter_addr_str[18];

	ba2str(&device->bdaddr, addr);
	DBG("Requesting agent authentication for %s", addr);

    adapter_get_address(device->adapter, &adapter_addr);
    ba2str(&adapter_addr, adapter_addr_str);
    read_device_name(adapter_addr_str, addr, device->name);
    DBG("device_request_authentication, device name is: %s", device->name);
