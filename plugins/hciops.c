...
static int hciops_set_powered(int index, gboolean powered)
{
	struct dev_info *dev = &devs[index];
	int err;
	/*only enable the following code when you find switch bt_quickly cause headset connection fail*/
	int fm_state = get_fm_state();

	while ((ENABLING_STATE == fm_state) || (DISABLING_STATE == fm_state)) {
		usleep(100000);
